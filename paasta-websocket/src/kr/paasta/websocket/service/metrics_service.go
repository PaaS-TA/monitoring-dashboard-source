...
func GetMetricsService(influxClient client.Client) *MetricsService {
...
func (b MetricsService) GetDiskIOList(request model.MetricsRequest) (model.DiskIOUsage, model.ErrMessage) {
	...
	if err != nil {
		...
		readUsage, _ := utils.GetResponseConverter().InfluxConverter(DiskIORdUsage, request.ServiceName)
		writeUsage, _ := utils.GetResponseConverter().InfluxConverter(DiskIOWrtUsage, request.ServiceName)
...
func (b MetricsService) GetNetworkIOList(request model.MetricsRequest) (model.NetworkIOUsage, model.ErrMessage) {
	...
	if err != nil {
		...
		rxUsage, _ := utils.GetResponseConverter().InfluxConverter(NetworkRxIOUsage, request.ServiceName)
		txUsage, _ := utils.GetResponseConverter().InfluxConverter(NetworkTxIOUsage, request.ServiceName)
...
func (b MetricsService) GetTopProcessList(request model.MetricsRequest) (model.TopProcessList, model.ErrMessage) {
	...
	if err != nil {
		...
		result, _ := utils.GetResponseConverter().InfluxConverter(resp, request.ServiceName)
...
func (b MetricsService) GetAppCpuUsage(request model.MetricsRequest) (model.AppNetworkIOKbyte, model.ErrMessage) {
	...
	resp, err := dao.GetMetricsDao(b.influxClient, "container_metric_db").GetAppCpuUsage(request)

	if err != nil {
		return jsonRes, err
	} else {
		result, err := utils.GetResponseConverter().InfluxConverter(resp, "cpu")
		if err != nil {
			return jsonRes, err
		}
...
func (b MetricsService) GetAppMemoryUsage(request model.MetricsRequest) (model.AppNetworkIOKbyte, model.ErrMessage) {
	jsonRes := model.AppNetworkIOKbyte{Name: "", Data: make([]map[string]interface{}, 1)}
	resp, err := dao.GetMetricsDao(b.influxClient, "container_metric_db").GetAppMemoryUsage(request)
	if err != nil {
		return jsonRes, err
	} else {
		result, err := utils.GetResponseConverter().InfluxConverter(resp, "memory")
		if err != nil {
			return jsonRes, err
		}
...
func (b MetricsService) GetDiskUsage(request model.MetricsRequest) (map[string]interface{}, model.ErrMessage) {
	...
	resp, err := dao.GetMetricsDao(b.influxClient, "container_metric_db").GetAppDiskUsage(request)
	if err != nil {
		return nil, err
	} else {
		result, err := utils.GetResponseConverter().InfluxConverter(resp, "disk")
		if err != nil {
			return nil, err
		}
		fmt.Println("#### Cpu variation :", result)
...
func (b MetricsService) GetApplicationResources(request model.MetricsRequest) (model.ApplicationResources, model.ErrMessage) {
	resp, err := dao.GetMetricsDao(b.influxClient, "container_metric_db").GetApplicationResources(request)

	var appResources model.ApplicationResources
	if err != nil {
		return appResources, err
	} else {
		result, _ := utils.GetResponseConverter().InfluxConverter(resp, "resources")
...
func (b MetricsService) GetApplicationResourcesAll(request model.MetricsRequest) (model.ApplicationResources, model.ErrMessage) {
	resp, err := dao.GetMetricsDao(b.influxClient, "container_metric_db").GetApplicationResourcesAll(request)
...
func (b MetricsService) GetAppNetworkKByte(request model.MetricsRequest) (model.AppNetworkIOKbyte, model.ErrMessage) {
	...
	resp_rx, err := dao.GetMetricsDao(b.influxClient, "container_metric_db").GetAppNetworkKByte(request, "rx_bytes")
	if err != nil {
		return result, err
	} else {
		result_rx, err = utils.GetResponseConverter().InfluxConverter(resp_rx, "Rx_Network")
		if err != nil {
			return result, err
		}
	...
	resp_tx, err := dao.GetMetricsDao(b.influxClient, "container_metric_db").GetAppNetworkKByte(request, "tx_bytes")
	if err != nil {
		return result, err
	} else {
		result_tx, err = utils.GetResponseConverter().InfluxConverter(resp_tx, "Tx_Network")
		if err != nil {
			return result, err
		}
