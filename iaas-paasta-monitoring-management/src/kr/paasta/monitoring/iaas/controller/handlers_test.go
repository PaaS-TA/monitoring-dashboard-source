...
func NewHandler(openstack_provider model.OpenstackProvider, iaasInfluxClient client.Client, paasInfluxClient client.Client,
	...
	var mainController *OpenstackServices
	var computeController *OpenstackComputeNode
	var manageNodeController *OpenstackManageNode
	var tenantController *OpenstackTenant
	var notificationController *NotificationController
	var definitionController *AlarmDefinitionController
	var stautsController *AlarmStatusController
	var logController *OpenstackLog

	var iaasActions rata.Handlers

	if sysType == utils.SYS_TYPE_IAAS || sysType == utils.SYS_TYPE_ALL {
		mainController = NewMainController(openstack_provider, iaasInfluxClient)
		computeController = NewComputeController(openstack_provider, iaasInfluxClient)
		manageNodeController = NewManageNodeController(openstack_provider, iaasInfluxClient)
		tenantController = NewOpenstackTenantController(openstack_provider, iaasInfluxClient)
		notificationController = NewNotificationController(monsClient, iaasInfluxClient)
		definitionController = NewAlarmDefinitionController(monsClient, iaasInfluxClient)
		stautsController = NewAlarmStatusController(monsClient, iaasInfluxClient, iaasTxn)
		logController = NewLogController(openstack_provider, iaasInfluxClient, iaasElasticClient)
