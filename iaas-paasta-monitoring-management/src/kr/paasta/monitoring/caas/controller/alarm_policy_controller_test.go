...
var _ = Describe("Alarm API", func() {
	...
	Describe("CaaS Alarm", func() {
		Context("Alarm Policy", func() {
			It("CAAS_ALARM_POLICY_LIST", func() {
				res, err := DoGet(testUrl + "/v2/caas/monitoring/alarmInfo")

				assert.Nil(tt, err)
				assert.Equal(tt, http.StatusOK, res.Code)
			})

			It("CAAS_ALARM_POLICY_UPDATE (PUT)", func() {
				...
				res, err := DoUpdate(testUrl+"/v2/caas/monitoring/alarmUpdate", TestToken, strings.NewReader(string(data)))

				assert.Nil(tt, err)
				assert.Equal(tt, http.StatusCreated, res.Code)
			})

			It("CAAS_ALARM_SNS_CHANNEL_LIST", func() {
				res, err := DoGet(testUrl + "/v2/caas/monitoring/snsChannel/list  ")

				assert.Nil(tt, err)
				assert.Equal(tt, http.StatusOK, res.Code)
			})

			It("CAAS_ALARM_SNS_CHANNEL_CREATE(POST)", func() {

				query := model.BatchAlarmSnsRequest{
					OriginType: "CaaS",
					SnsId:      "CaaSMonitoringBot",
				...
				res, err := DoPost(testUrl+"/v2/caas/monitoring/alarmSnsSave", TestToken, strings.NewReader(string(data)))
				assert.Nil(tt, err)
				assert.Equal(tt, http.StatusCreated, res.Code)
			})
			//
			It("CAAS_ALARM_SNS_CHANNEL_DELETE(DELETE)", func() {
				res, err := DoDelete(testUrl+"/v2/caas/monitoring/snsChannel/"+channel_id, TestToken, strings.NewReader(string("")))
				fmt.Println(res)
				assert.Nil(tt, err)
				assert.Equal(tt, http.StatusOK, res.Code)
			})
