...
func (a *AutoScalerStruct) setResourceUsage(appInfo *model.ApplicationInfo, policy model.AppAutoScalingPolicy, listAutoScaleTarget *[]model.AutoScaleTarget) {
	...
	for _, container := range appInfo.ApplicationContainerInfo {
		...
		if policy.AutoScalingOutYn == "Y" {
			...
			if container.CpuUsage > float64(policy.CpuMaxThreshold) && cfApp.Instances < int(policy.InstanceMaxCnt) && !isAppended {
			...
			if container.MemoryUsage > float64(policy.MemoryMaxThreshold) && cfApp.Instances < int(policy.InstanceMaxCnt) && !isAppended {
		...
		if policy.AutoScalingInYn == "Y" {
			...
			if container.CpuUsage < float64(policy.CpuMinThreshold) && cfApp.Instances > int(policy.InstanceMinCnt) && !isAppended {
			...
			if container.MemoryUsage < float64(policy.MemoryMinThreshold) && cfApp.Instances > int(policy.InstanceMinCnt) && !isAppended {
