...
func AuthenticatedClient(options gophercloud.AuthOptions) (*gophercloud.ProviderClient, error) {
	//fmt.Println("AuthenticatedClient 000 ::options.TokenID=",options.TokenID)
    ...
    //fmt.Println("AuthenticatedClient 111 ::client.TokenID=",client.TokenID)
	err = Authenticate(client, options)
	if err != nil {
		return nil, err
	}

	//fmt.Println("AuthenticatedClient 222 ::client.TokenID=",client.TokenID)
	return client, nil
}
...
func v3auth(client *gophercloud.ProviderClient, endpoint string, options gophercloud.AuthOptions) error {
	...
	var result tokens3.CreateResult
		...
		if(client.TokenID =="") {
			...
			//bool,err := tokens3.Validate(v3Client, client.TokenID)
			//if err != nil {
			//	return err
			//}
			result2 := tokens3.Get(v3Client, client.TokenID)
