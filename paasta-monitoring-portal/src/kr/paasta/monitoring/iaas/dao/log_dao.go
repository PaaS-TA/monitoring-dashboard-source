...
func (log LogDao) GetDefaultRecentLog(request model.LogMessage, paging bool) (_ model.LogMessage, errMsg model.ErrMessage) {
	...
	if request.Index == "" {
		...
		now := time.Now().Local()
		//current := now.Unix() - int64(model.GMTTimeGap)*60*60 //9 hour difference Between Local PC and GMT(Greenwich Mean Time).
		current := now.Unix()
		/*
		...
		*/
		...
		//before = before - int64(model.GMTTimeGap)*60*60 //9시간	=> if time zone is equal to Logsearch Instance, it must be removed.
				...
				if err != nil {
					...
					for key, value := range rawData {
						if strings.Contains(key, "timestamp") {
							...
							logInfo.Time = time.Unix(convert_time.Unix()+int64(model.GMTTimeGap)*60*60, 0).Format(time.RFC3339)[0:19]
						}
