...
func (p *ApPaastaService) GetPaastaLog(paastaLog models.PaastaLog) ([]models.PaastaLog, error) {
	var results []models.PaastaLog

	if paastaLog.Period == "" {

		if paastaLog.StartTime == "" && paastaLog.EndTime == "" {
			paastaLog.StartTime = fmt.Sprintf("%sT%s", paastaLog.TargetDate, "00:00:00")
			paastaLog.EndTime = fmt.Sprintf("%sT%s", paastaLog.TargetDate, "23:59:59")
		} else if paastaLog.StartTime != "" && paastaLog.EndTime == "" {
			paastaLog.StartTime = fmt.Sprintf("%sT%s", paastaLog.TargetDate, paastaLog.StartTime)
			paastaLog.EndTime = fmt.Sprintf("%sT%s", paastaLog.TargetDate, "23:59:59")
		} else if paastaLog.StartTime == "" && paastaLog.EndTime != "" {
			paastaLog.StartTime = fmt.Sprintf("%sT%s", paastaLog.TargetDate, "00:00:00")
			paastaLog.EndTime = fmt.Sprintf("%sT%s", paastaLog.TargetDate, paastaLog.EndTime)
		} else {
			paastaLog.StartTime = fmt.Sprintf("%sT%s", paastaLog.TargetDate, paastaLog.StartTime)
			paastaLog.EndTime = fmt.Sprintf("%sT%s", paastaLog.TargetDate, paastaLog.EndTime)
		}
		convert_start_time, _ := time.Parse(time.RFC3339, fmt.Sprintf("%s+09:00", paastaLog.StartTime))
		convert_end_time, _ := time.Parse(time.RFC3339, fmt.Sprintf("%s+09:00", paastaLog.EndTime))
		...
		paastaLog.StartTime = time.Unix(startTime, 0).Format(time.RFC3339)[0:19] + ".000000000Z"
		paastaLog.EndTime = time.Unix(endTime, 0).Format(time.RFC3339)[0:19] + ".000000000Z"
	}
	logResp, err := dao.GetPaastaDao(p.DbInfo, p.InfluxDbClient).GetPaastaLog(paastaLog)
	if err != nil {
		fmt.Println(err.Error())
		return results, err
	}
	...
	var resultPaastaLog models.PaastaLog
	resultPaastaLog.UUID = paastaLog.UUID
	resultPaastaLog.LogType = paastaLog.LogType
	resultPaastaLog.Keyword = paastaLog.Keyword
	resultPaastaLog.TargetDate = paastaLog.TargetDate
	resultPaastaLog.Period = paastaLog.Period
	resultPaastaLog.StartTime = paastaLog.StartTime
	resultPaastaLog.EndTime = paastaLog.EndTime
	resultPaastaLog.Messages = messages["metric"]
	results = append(results, resultPaastaLog)
