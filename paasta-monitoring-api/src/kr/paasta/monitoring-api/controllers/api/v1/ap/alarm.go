...
func (ap *ApAlarmController) GetAlarmStatus(c echo.Context) error {
	results, err := AP.GetApService(ap.DbInfo).GetAlarmStatus()
	if err != nil {
...
func (ap *ApAlarmController) GetAlarmPolicy(c echo.Context) error {
	results, err := AP.GetApService(ap.DbInfo).GetAlarmPolicy()
	if err != nil {
...
func (ap *ApAlarmController) UpdateAlarmPolicy(c echo.Context) error {
	var request []models.AlarmPolicyRequest // 클라이언트의 리퀘스트 정보를 저장할 변수 선언
	err := c.Bind(&request)
	//err := helpers.BindRequestAndCheckValid(c, &request) // 클라이언트의 리퀘스트 정보의 바인딩 & 유효성 결과를 반환
	//if err != nil {
	//	apiHelpers.Respond(c, http.StatusBadRequest, "Invalid JSON provided, please check the request JSON", err.Error())
	//	return err
	//}

	results, err := AP.GetApService(ap.DbInfo).UpdateAlarmPolicy(request)
	if err != nil {
