...
angular.module('monitor.controllers')
    ...
    .controller('almSetCtrl', function($scope, $timeout, $exceptionHandler, common, almSrvc) {
            ...
            containerPromise.success(function (result) {
                var setup = {
                    pas: {
                        ...
                        email: {emailName: '', emailAddr: '', emailType: ''}
                    },
                    bos: {
                        cpu: {threshold: {critical: 0, warning: 0}, repeatTime: 0},
                        memory: {threshold: {critical: 0, warning: 0}, repeatTime: 0},
                ...
                for(var i in result) {
                    ...
                    // Mail Setting
                    var mailAddress2 = 'honggildong@zesinc.com';
                    //if (mailAddress2.indexOf("@") > -1) {
                    //    var array = mailAddress2.split('@');
                    if (result[i].mailAddress.indexOf("@") > -1) {
                        var array = result[i].mailAddress.split('@');
                        var vBool = true;
                        var vType = array[1];
                        for (var j = 0; j < $scope.mailList.length; j++) {
                            if ($scope.mailList[j].code == vType) { vBool = false; }
                        }
                        if (vBool) { vType = ''; }
                        setup[result[i].originType].email = {
                            emailName: array[0],
                            emailAddr: array[1],
                            emailType: vType
                        }
        ...
        $scope.setEmailAddr = function() {
            if($scope.emailType == '') {
                $scope.origin.email.emailAddr = '';
            } else {
                $scope.origin.email.emailAddr = $scope.origin.email.emailType;
            }
        };

        $scope.saveSetup = function() {
            $scope.loading = true;
            if(!($scope.origin.email.emailName != '' && $scope.origin.email.emailAddr != '')) {
                $scope.loading = false;
                var vMsg = '';
                if ($scope.origin.email.emailName == '') {
            ...
            var body = [
                ...
                {
                    originType: $scope.selType,
                    mailAddress: $scope.origin.email.emailName + '@' + $scope.origin.email.emailAddr
                }
            ];
            ...
            containerPromise.error(function (reason, status, headers) {
                $scope.loading = false;
                $timeout(function() { $exceptionHandler(reason, {code: status, message: reason}); }, 500);
            });
        };
    })
